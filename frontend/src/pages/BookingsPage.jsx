...
function BookingsPage({ mode = "my" }) {
  ...
  return (
    <OperationsShell
      title={isCreateView ? "Create Booking" : isAdminReviewView ? "Admin Bookings" : "My Bookings"}
      subtitle="Manage your facility and resource reservations"
      navItems={navItems}
    >
      <div className={`ops-content booking-page${isAdminReviewView || isCreateView ? " booking-page-admin" : ""} ${isStudentBookingsView ? "min-h-screen rounded-3xl bg-slate-50 p-4 md:p-6" : ""}`}>
                ...
                isStudentBookingsView ? (
                  <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center">
                    ...
                    <button
                      ...
                      className="mt-6 rounded-xl bg-violet-300 px-6 py-2.5 text-sm font-semibold text-violet-900 transition hover:bg-violet-400"
                    >
                  ...
                  <div className="booking-empty">
                    <p>No bookings found matching your current filter set.</p>
                  </div>
                )
              ) : (
                  ...
                  <table className={`${isStudentBookingsView ? "min-w-195 w-full border-separate border-spacing-0" : `booking-table${isAdminReviewView ? " booking-table-admin" : ""}`}`}>
                    ...
                    <tbody>
                      {visibleBookings.map((booking) => {
                        const canCancel = !isAdmin && booking.userId === currentUserId && booking.status === "PENDING";
